...
namespace RecommenderAttacksAnalytics.InputOutput
{
    public class TextFileDataIO : AbstractDataIO
    {
        ...
        private void fileWriteWork(TextWriter writer)
        {
            var userItemRatings = RatingsLookupTable.Instance.getAllUserItemRatings();
            foreach (var userItemRating in userItemRatings)
            {
                var argsAsString = formatUserItemRatingPairToString(userItemRating.Key, userItemRating.Value);
                writer.WriteLine(argsAsString);
            }
            writer.Close();
        }


        private string formatUserItemRatingPairToString(UserItemPair userItemPair,  int rating)
        {
            return String.Format("{0} {1} {2}", userItemPair.User.getId(), userItemPair.Item.getId(), rating);
        }

        private void fileReadWork(TextReader reader)
        {
            string currentLine;
            m_entriesProcessed = 0;
            long catchUpValue = m_numberOfUserItemPairs / 100;
            ...
            while ((currentLine = reader.ReadLine()) != null)
            {
                ...
                // Used to better report progress at each percentage completion
                if (catchUpValue == 0 || m_entriesProcessed == 1 || (m_entriesProcessed % catchUpValue) == 0 || completionPercentage == 100)
                    System.Threading.Thread.Sleep(1);
            }
